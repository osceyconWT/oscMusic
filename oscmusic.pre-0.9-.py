...
def sort_album_songs(songs):
    return sorted(songs, key=lambda x: (x[1]["tracknumber"], x[1]["title"]))
...
def main(stdscr, playlist):
    ...
    while True:
        ...
        if in_album_view:
            stdscr.addstr(0, 1, "oscMusic player - v0.9 CLOSED BETA - an oscey project")
            stdscr.addstr(1, 1, "your albums | arrow keys to nav, enter to select, q to quit")
            ...
            stdscr.addstr(2, 1, f"album - {metadata['album']}")
            ...
            for i in range(scroll_offset, min(scroll_offset + max_display, len(song_list))):
                ...
                song_info = f"{song_metadata['title']} - {song_metadata['artist']}"
...
if __name__ == "__main__":
    playlist_dir = "/home/oscey/Music"  
    playlist = [os.path.join(playlist_dir, f) for f in os.listdir(playlist_dir) if f.endswith(".mp3")]
